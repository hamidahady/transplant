...
pred_func<-function(train,hold_out,formul,pred_type,method_sam){
 ...
 # I used 5 fold cross validation with 3 repeats
  ...
  rownames(resul_perf)<-c("auc","sen","spec","accu")
      ...
      resul_raw$rf<-predict(mod_rf_stack, newdata=hold_out, type="raw")
      resul_prob$rf<-predict(mod_rf_stack, newdata=hold_out, type="prob")[,2]
...
resul_year0_RUS<-pred_func(train,hold_out,year0,"regular","RUS")

formul<-as.formula(paste("rf"," ~ ",paste(c("log","svm","nnet"), collapse="+"),sep = ""))
resul_year0_RUS<-pred_func(train,hold_out,formul,"stacking","NONE")

resul_year0$resul_perf
