...
# 2001-2005
# 2006-
temp<-heart.df.cleaned$TX_YEAR
heart.df.cleaned$TX_YEAR<-temp

# categoirizing the transplantation date
for(i in 1:nrow(heart.df.cleaned)){
  if(!is.na(heart.df.cleaned$TX_YEAR[i])){ 
    if(heart.df.cleaned$TX_YEAR[i]<=1995){heart.df.cleaned$TX_YEAR[i]<-"A"}
    else if(heart.df.cleaned$TX_YEAR[i]>1995 && heart.df.cleaned$TX_YEAR[i]<=2000){heart.df.cleaned$TX_YEAR[i]<-"B"}
    else if(heart.df.cleaned$TX_YEAR[i]>2000 && heart.df.cleaned$TX_YEAR[i]<=2005){heart.df.cleaned$TX_YEAR[i]<-"C"}
    else if(heart.df.cleaned$TX_YEAR[i]>2005){heart.df.cleaned$TX_YEAR[i]<-"D"}
        }
}
 
table(heart.df.cleaned$TX_YEAR)
str(heart.df.cleaned$TX_YEAR)
#Here save4
...
vars<-c("AGE","AGE_DON","HGT_CM_CALC","HGT_CM_DON_CALC","GENDER","GENDER_DON","WGT_KG_DON_CALC","WGT_KG_TCR","DIAG","COD_CAD_DON", 
        "ABO","ABO_DON","DIAB","INFECT_IV_DRUG_TRR","TRANSFUSIONS","PREV_TX","CARD_SURG","ICU","VAS","VENTILATOR_TRR",
        "ECMO","HEMO_SYS_TRR","PVR","ISCHTIME","CREAT_TRR","TBILI","PRAMR","RDR2","TX_YEAR",
