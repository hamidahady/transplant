...
heart.df.cleaned <- subset(thoracic_data,WL_ORG=="HR") %>% 
 ...
 # subset(is.na(GTIME)==FALSE)  %>% 
  ...
  subset(INIT_AGE>=18)  %>% #excluding kids,
...
heart.df.cleaned$PRIOR_CARD_SURG_TCR
heart.df.cleaned$PRIOR_CARD_SURG_TRR
...
# Mechanical ventilation / merge of (VENT_SUPPORT_AFTER_LIST, VENT_SUPPORT_TRR, VENTILATOR_TCR, VENTILATOR_TRR, ONVENT)
# all of UNOS data has Y and N however in "VENTILATOR_TCR" Y=1 and N=0
...
## find categorical variables with SAS.ANALYSIS.FORMAT: TH_DGN
## Group1: 1000-1049; Group2: 1050-1099; Group3: 1100-1199; Group4: 1200- ; Group5: 999
TH_DGN.index <- which(new.form$SAS.ANALYSIS.FORMAT=="TH_DGN")
for (i in 1:length(TH_DGN.index)){
  one.vector <- as.numeric(as.character(heart.df.cleaned[,TH_DGN.index[i]]))
  new.vector <- rep(NA, length(one.vector))
  for (j in 1:length(one.vector)){
    if (one.vector[j]>=1000 & one.vector[j]<=1049) new.vector[j] <- "G1"
    if (one.vector[j]>=1050 & one.vector[j]<=1099) new.vector[j] <- "G2"
    if (one.vector[j]>=1100 & one.vector[j]<=1199) new.vector[j] <- "G3"
    if (one.vector[j]>=1200) new.vector[j] <- "G4"
    if (one.vector[j]==999) new.vector[j] <- "G5"
  }
  heart.df.cleaned[,TH_DGN.index[i]] <- factor(new.vector)
}
...
# Find identifiers
find.uniqueness <- apply(heart.df.cleaned, 2, function(x) length(unique(x)))
ID.cols <- as.vector(colnames(heart.df.cleaned)[which(find.uniqueness==nrow(heart.df.cleaned))])
