...
TARGET<-"year0"
 ...
 # I used 5 fold cross validation with 3 repeats
  ...
  names(resul_raw)<-c("log","svm","nnet","rf")
  resul_prob<-resul_raw
  
  resul_perf<-as.data.frame(matrix(NA, ncol = 4, nrow = 4))
  names(resul_perf)<-c("log","svm","nnet","rf")
  rownames(resul_perf)<-c("auc","sen","spec","accu")
  

    if(pred_type=="regular"){
      mod_log <-caret::train(formul,  data=train, method="glm", family="binomial",
                           trControl = control_, tuneLength = 10, metric="ROC")
 
      mod_svm<-caret::train(formul,  data=train, method="svmRadial", family="binomial",
                          trControl = control_,  tuneGrid=expand.grid(C=svm_cost, sigma=svm_sigma),
                          tuneLength = 10)

      mod_nnet<-caret::train(formul,  data=train, method="nnet", family="binomial",
                           trControl = control_,  tuneGrid=expand.grid(size=5, decay=0.1), MaxNWts=20000,
                           tuneLength = 10)
      
      
      resul_raw$log<-predict(mod_log, newdata=hold_out, type="raw")
      resul_prob$log<-predict(mod_log, newdata=hold_out, type="prob")[,2]

      resul_raw$svm<-predict(mod_svm, newdata=hold_out, type="raw")
      resul_prob$svm<-predict(mod_svm, newdata=hold_out, type="prob")[,2]
 
      resul_raw$nnet<-predict(mod_nnet, newdata=hold_out, type="raw")
      resul_prob$nnet<-predict(mod_nnet, newdata=hold_out, type="prob")[,2]
      
      }
  
  
     if(pred_type=="regular" | pred_type=="stacking"){
      mod_rf<-caret::train(formul,  data=train, method="rf",
                        trControl = control_, tuneGrid=expand.grid(mtry = ncol(train)-4),
                        tuneLength = 10)
       
      resul_raw$rf<-predict(mod_rf, newdata=hold_out, type="raw")
      resul_prob$rf<-predict(mod_rf, newdata=hold_out, type="prob")[,2]
    ...
    resul_perf["sccu","log"]<-(as.data.frame(confusionMatrix(resul_raw["log"],hold_out[TARGET])$overall))[1,]
