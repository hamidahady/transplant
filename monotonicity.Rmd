...
# table(heart.df.cleaned$PRAMR)
# table(heart.df.cleaned$PRIOR_CARD_SURG_TRR)
# 
# table(thoracic_data$PRIOR_CARD_SURG_TCR)
# table(thoracic_data$PRIOR_CARD_SURG_TRR)
nrow(as.data.frame(table(thoracic_data$ID)))
...
sum(as.data.frame(table(heart.df.cleaned$PRAMR))[2])

for(i in 1:nrow(heart.df.cleaned)){
  if(!is.na(heart.df.cleaned$PRAMR[i])){ 
    if(heart.df.cleaned$PRAMR[i]>10){heart.df.cleaned$PRAMR[i]<-"Y"}
    if(heart.df.cleaned$PRAMR[i]<=10){heart.df.cleaned$PRAMR[i]<-"N"}
        }
  }
