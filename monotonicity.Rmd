...
library(pacman) 
p_load(caret,httr, DT,stringr,mlr,AUC)
...
#str(heart_max)
  ...
  assign(paste("year",i,sep=""),as.formula(formula_format))
  
  }


 # I used 5 fold cross validation with 3 repeats
  control_<- trainControl(method = "repeatedcv",  savePredictions = TRUE, number=5, repeats = 3,
 ...
 train<-RUS_func(train,paste("year",i,sep=""))
    
mod_log <-caret::train(eval(parse(text = paste("year",i,sep = ""))),  data=train, method="glm", family="binomial",
                           trControl = control_, tuneLength = 10, metric="ROC")
 
mod_svm<-caret::train(eval(parse(text = paste("year",i,sep = ""))),  data=train, method="svmRadial", family="binomial",
                          trControl = control_,  tuneGrid=expand.grid(C=svm_cost, sigma=svm_sigma),
                          tuneLength = 10)

mod_nnet<-caret::train(eval(parse(text = paste("year",i,sep = ""))),  data=train, method="nnet", family="binomial",
                           trControl = control_,  tuneGrid=expand.grid(size=5, decay=0.1), MaxNWts=20000,
                           tuneLength = 10)

mod_rf<-caret::train(eval(parse(text = paste("year",i,sep = ""))),  data=train, method="rf",
                        trControl = control_, tuneGrid=expand.grid(mtry = ncol(train)-4),
                        tuneLength = 10)


 
 resul_log_raw<-predict(mod_log, newdata=hold_out, type="raw")
 resul_log_prob<-predict(mod_log, newdata=hold_out, type="prob")[,2]
 
 resul_log_raw<-predict(mod_svm, newdata=hold_out, type="raw")
 resul_log_prob<-predict(mod_svm, newdata=hold_out, type="prob")[,2]
 
 resul_log_raw<-predict(mod_nnet, newdata=hold_out, type="raw")
 resul_log_prob<-predict(mod_nnet, newdata=hold_out, type="prob")[,2]
 
 resul_log_raw<-predict(mod_rf, newdata=hold_out, type="raw")
 resul_log_prob<-predict(mod_rf, newdata=hold_out, type="prob")[,2]

 auc_temp<-AUC:: auc(roc(resul_log_prob,hold_out$year0))
 sen_temp<-caret:: sensitivity(resul_log_raw,hold_out$year0)
 spe_temp<-caret:: specificity(resul_log_raw,hold_out$year0)
 acu_temp<-(as.data.frame(confusionMatrix(resul_log_raw,hold_out$year0)$overall))[1,]
