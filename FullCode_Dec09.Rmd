...
# we do not need anything on the beow variable based on ref1
 ...
 base[ c("TCR_DGN", "DIAG","THORACIC_DGN")]<-NULL
 ...
 base[c("DEATH_CIRCUM_DON","DEATH_MECH_DON")]<-NULL
...
heart.df.cleaned<-cat_changer(heart.df.cleaned,var="HLAMIS",val_old,val_new)
  ...
  base[c("COD_CAD_DON","ABO","ABO_DON","DIAB","PRIOR_CARD_SURG_TCR","PRIOR_CARD_SURG_TRR",
       "DA1","DA2","RA1","RA2","DB1","DB2","RB1","RB2","RDR1","RDR2","DDR1","DDR2","HLAMIS","AMIS","BMIS","DRMIS",
       "TX_YEAR")]<-NULL
 ...
 # the following block is our variable manipulation based on the other literature as specified in Ali Dag's paper
  ...
  base[c("ETHCAT","ETHCAT_DON","GENDER","GENDER_DON","PROC_TY_HR","SHARE_TY","EDUCATION",
       "ETHNICITY","PRI_PAYMENT_CTRY_TCR", "PRI_PAYMENT_CTRY_TRR","PRI_PAYMENT_TCR", "PRI_PAYMENT_TRR",
       "REGION","FUNC_STAT_TRR","FUNC_STAT_TCR")]<-NULL
  
  # now, I finished the literature variables, I investigate the other variables bymyself
  # following variables has too many NAs, so they are dropped
  
 heart.df.cleaned[ c("WLHL", "WLHR", "WLIN" , "WLKI", "WLKP" ,"WLLI", "WLLU", "WLPA","WLPI","VAD_AT_LISTING",
                     "VAD_WHILE_LISTED","VESSELS_50STEN","WALL_ABN_GLOB_DON","WALL_ABN_SEG_DON" ,"RECOV_COUNTRY",
                     "PT_OTH4_OSTXT_DON","PT_OTH3_OSTXT_DON","OTHER_INF_OSTXT_DON","OTHER_INF_CONF_DON",
                     "MALIG_TY_OSTXT","MALIG_TY_OSTXT_TCR","INSULIN_DUR_DON","INIT_O2","HIST_INSULIN_DEP_DON",
                     "HCV_ANTIBODY_DON","HCV_RIBA_DON","HCV_RNA_DON","HCV_TEST_DON","HBV_TEST_DON",
                     "HBV_SUR_ANTIGEN_DON","HBV_DNA_DON","GROUPING","FEV1_TRR","FVC_TRR","END_PRIORITY",
                     "END_O2","EBV_DNA_DON","EBV_IGG_DON","EBV_IGM_DON","EBV_TEST_DON","CONTROLLED",
                     "CANCER_FREE_INT_DON","BLOOD_INF_CONF_DON","CALC_LAS_LISTDATE","DIAG_OSTXT",
                     "INIT_PRIORITY","INIT_CREAT","CMV_IGG_DON", "CMV_IGM_DON", "CMV_NUCLEIC_DON",
                     "CMV_TEST_DON", "COD_LIV_DON" ,"COD_OSTXT_DON","DIABDUR_DON","EXERCISE_O2" ,"LIV_DON_TY")]<-NULL
 ...
 heart.df.cleaned[ c("WL_ID_CODE", "WL_ORG","INIT_DATE","TX_DATE","CTR_CODE","DATA_TRANSPLANT",
                     ...
                     "MALIG_TY","MALIG_TY_TCR")]<-NULL
 
 #the following variables(s) almost all the patients are in one category
 #age group is "A" because we already selected the adult patients
 # I checked them in the entire dataset , too. so they may not even in our limited observations but I considered
 # them as almost one-level categorical variable because of the original large dataset
 heart.df.cleaned[ c("HIST_MI","END_BLU_FLG","END_CREAT","END_LLU_FLG","END_RLU_FLG","EDUCATION_DON",
                     "CITIZENSHIP","CITIZENSHIP_DON","CANCER_SITE_DON","HIST_CANCER_DON",
                     "SKIN_CANCER_DON","EXTRACRANIAL_CANCER_DON","INTRACRANIAL_CANCER_DON","AGE_GROUP","BIOPSY_DGN",
                     "DIURETICS_DON","INIT_RLU_FLG","INIT_LLU_FLG","INIT_BLU_FLG","DON_TY","HIV_SEROSTATUS","HTLV2_OLD_DON",
                     "INHALED_NO", "INHALED_NO_TRR","LVAD_AT_LISTING","LVAD_WHILE_LISTED")]<-NULL
 
 base[ c("WLHL", "WLHR", "WLIN" , "WLKI", "WLKP" ,"WLLI", "WLLU", "WLPA","WLPI","VAD_AT_LISTING",
         "VAD_WHILE_LISTED","VESSELS_50STEN","WALL_ABN_GLOB_DON","WALL_ABN_SEG_DON" ,"WL_ID_CODE", "WL_ORG",
         "RECOV_COUNTRY","PT_OTH4_OSTXT_DON","PT_OTH3_OSTXT_DON","OTHER_INF_OSTXT_DON","OTHER_INF_CONF_DON",
         "MALIG_TY_OSTXT","MALIG_TY_OSTXT_TCR","INSULIN_DUR_DON","INIT_O2","HIST_INSULIN_DEP_DON",
         "HIST_MI","HCV_ANTIBODY_DON","HCV_RIBA_DON","HCV_RNA_DON","HCV_TEST_DON",
         "HBV_TEST_DON","HBV_SUR_ANTIGEN_DON","HBV_DNA_DON","GROUPING","FEV1_TRR","FVC_TRR","END_PRIORITY","END_BLU_FLG",
         "END_CREAT","END_LLU_FLG","END_O2","END_RLU_FLG","EBV_DNA_DON","EBV_IGG_DON","EBV_IGM_DON","EBV_TEST_DON",
         "EDUCATION_DON","CONTROLLED","CITIZENSHIP","CITIZENSHIP_DON","CANCER_FREE_INT_DON","CANCER_SITE_DON",
         "HIST_CANCER_DON","SKIN_CANCER_DON","EXTRACRANIAL_CANCER_DON","INTRACRANIAL_CANCER_DON","AGE_GROUP","BIOPSY_DGN",
         "BLOOD_INF_CONF_DON","CALC_LAS_LISTDATE","DIAG_OSTXT","DIURETICS_DON","INIT_RLU_FLG","INIT_PRIORITY","INIT_LLU_FLG",
         "INIT_DATE","INIT_CREAT","INIT_BLU_FLG","TX_DATE","CMV_IGG_DON", "CMV_IGM_DON","CMV_NUCLEIC_DON","CMV_TEST_DON",
         "COD_LIV_DON", "COD_OSTXT_DON" 
         )]<-NULL
...
heart.df.cleaned<-cat_changer(heart.df.cleaned,var="LAST_INACT_REASON",val_old,val_new)
 ...
 base[ c("ABO_MAT", "ANTIHYPE_DON" , "BLOOD_INF_DON","BRONCHO_LT_DON","BRONCHO_RT_DON","CARDARREST_NEURO",
         "CHEST_XRAY_DON", "CLIN_INFECT_DON","CMV_DON","BUN_DON","CONTIN_CIG_DON", "CONTIN_COCAINE_DON", "CONTIN_OTH_DRUG_DON",          
          "CORONARY_ANGIO","CREAT_DON","CRSMATCH_DONE","CTR_CODE","DATA_TRANSPLANT","DATA_WAITLIST", "DAYS_STAT1",
         "DAYS_STAT1A","DAYS_STAT1B","DAYS_STAT2","DAYSWAIT_CHRON","DDAVP_DON","DIABDUR_DON","DIABETES_DON","DIAL_AFTER_LIST",
         "DIAL_PRIOR_TX","DIET_DON","DISTANCE","DON_RETYP","DON_TY","EBV_SEROSTATUS","ECD_DONOR", "END_BMI_CALC",
         "END_OPO_CTR_CODE","END_STAT","EXERCISE_O2","GSTATUS","GTIME","HBV_CORE","HBV_CORE_DON","HBV_SUR_ANTIGEN","HCV_SEROSTATUS",
         "HEMO_CO_TCR","HEMO_PA_DIA_TCR","HEMO_PA_DIA_TRR","HEMO_PA_MN_TCR","HEMO_PCW_TCR","HEMO_SYS_TCR","HEP_C_ANTI_DON",
         "HEPARIN_DON","HIST_CIG_DON","HIST_COCAINE_DON","HIST_DIABETES_DON", "HIST_HYPERTENS_DON", "HIV_SEROSTATUS", "HIST_OTH_DRUG_DON",
          "HOME_STATE_DON","HTLV2_OLD_DON","HYPERTENS_DUR_DON","IABP_TCR" ,"IABP_TRR","IMPL_DEFIBRIL","INHALED_NO","INHALED_NO_TRR","INIT_STAT",
         "INIT_OPO_CTR_CODE", "INOTROP_VASO_CO_TRR","INOTROP_VASO_DIA_TRR","INOTROP_VASO_MN_TRR", "INOTROP_VASO_PCW_TRR","INOTROP_VASO_SYS_TCR",
         "INOTROP_VASO_SYS_TRR","INOTROPES_TCR","INOTROPES_TRR", "INOTROPIC","ISCHTIME","LAST_INACT_REASON","LIFE_SUP_TRR", "LIFE_SUP_TCR",
         "LISTING_CTR_CODE","LIV_DON_TY","LOS","LV_EJECT", "LV_EJECT_METH","LVAD_AT_LISTING","LVAD_WHILE_LISTED","MALIG", "MALIG_TCR",
         "MALIG_TY","MALIG_TY_TCR","MED_COND_TRR")]<-NULL
 
names(base[1:10])      

table(heart.df.cleaned$LOS)  
table(heart.df.cleaned$LV_EJECT)
table(heart.df.cleaned$LV_EJECT_METH)
table(heart.df.cleaned$LVAD_AT_LISTING)  
table(heart.df.cleaned$LVAD_WHILE_LISTED)

table(heart.df.cleaned$MALIG) 
table(heart.df.cleaned$MALIG_TCR)
table(heart.df.cleaned$MALIG_TY)
table(heart.df.cleaned$MALIG_TY_TCR)             
table(heart.df.cleaned$MED_COND_TRR)
 

sum(as.data.frame(table(heart.df$MALIG_TY))[2])
sum(as.data.frame(table(heart.df.cleaned$MALIG_TY))[2])
table(heart.df$MALIG_TY)
table(heart.df$END_STAT)
table(heart.df$SKIN_CANCER_DON)
table(heart.dfWLKP)
table(heart.df$WLLI)
table(heart.df$WLLU)
table(heart.df$WLPA)
...
table(heart.df$REGION)
table(heart.df$DA2)
table(heart.df$PROC_TY_HR)

table(heart.df$HLAMIS)
